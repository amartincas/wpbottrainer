...
**Fix real post-E2E: localización más allá de la página 1 (ver D039)** — `YMoveExerciseProvider::fetchAndLocate()` (usada por `find()`/`resolveMedia()`/`variants()`) recorre TODAS las páginas del catálogo en modo browse (gratis) para localizar un `provider_exercise_id` concreto, en vez de mirar solo la primera (~20 de 1068 ítems, el bug real que dejaba sin video a cualquier ejercicio fuera de esa página). Solo si el llamador necesita video, se repite la ÚNICA página exacta donde se confirmó el id, ahora con `includeVideos=true` — nunca el catálogo completo. Encapsulado 100% en el adapter; `ExerciseProviderInterface`/`MediaResolver`/`TrainingEngine`/`TrainingHandler` no cambiaron.
