...
3. Si no, decide el **foco de continuidad** (`decideFocus()`, sin cambios desde Hito 4) evaluando, en orden de prioridad: (a) recuperación — un grupo muscular no trabajado hace más de `RECOVERY_NEGLECT_DAYS` tiene prioridad; (b) si la última sesión fue omitida, se reofrece el mismo foco; (c) evitar repetir el foco de la última sesión completada; (d) `TrainingProfile.next_focus` como señal de continuidad — **última prioridad, nunca autoridad absoluta**. Este foco (grupos musculares gruesos, `Exercise.muscle_group`) sigue existiendo sin cambios — pasa a ser el nivel "general" de la selección (punto 4).
4. **Selecciona hasta 3 ejercicios** (`selectExercises()`, reescrito en Hito 8.4 — ver D034) en el orden de prioridad **elegibilidad → foco declarado → objetivo/nivel → anti-repetición → determinismo**:
   - **Elegibilidad** (filtro duro, `isEligible()`): excluye cualquier ejercicio cuyas `contraindications` choquen con `restrictions`, o cuyo `equipment_needed` no esté cubierto por `available_equipment` — salvo que `TrainingProfile.equipment_fully_equipped=true`, que hace elegible cualquier ejercicio en cuanto a equipo. Desde Hito 9.0, `training_location=outdoor` excluye cualquier ejercicio con `equipment_needed !== []` sin excepción — lo que el usuario posee no es lo que tiene consigo al aire libre.
   - **Foco declarado** (Hito 8.4): entre los elegibles, 3 niveles estrictamente ordenados — primario (`Exercise.primary_muscle`/`secondary_muscles` intersecta `TrainingProfile.primary_focus`), secundario (ídem con `secondary_focus`), general (el foco de continuidad del punto 3, sin cambios). Sin foco declarado (`primary_focus === []`), el comportamiento es idéntico al de antes de Hito 8.4 — solo existe el nivel general.
   - **Objetivo/nivel**: dentro de un mismo nivel de foco, se prioriza el ejercicio cuyo `difficulty_level` coincide con `experience_level` del perfil.
   - **Anti-repetición**: penaliza (nunca excluye) un ejercicio usado en las 2 sesiones más recientes — solo desempata dentro de un mismo nivel de foco y de ajuste de nivel, nunca puede hacer ganar a un ejercicio peor en esas dos dimensiones por ser distinto.
   - **Determinismo**: último desempate por `id` ascendente.
   - Con `primary_focus` declarado, se espera un mínimo de `ceil(3/2)=2` ejercicios de los niveles primario+secundario combinados; si el catálogo elegible no alcanza, se registra `Log::info('TRAINING_FOCUS_FALLBACK', ...)` — nunca se bloquea la sesión ni se inventa un ejercicio.
5. Prescribe cada ejercicio: sin historial previo, valores de arranque dependientes del objetivo (`GOAL_DEFAULTS`, Hito 8.4 — heurísticas de producto documentadas en código, revisables, nunca una prescripción científica); con historial, progresión simple sin cambios (sube carga/duración si el RPE reportado fue manejable, se mantiene si fue alto). `rest_seconds` siempre refleja `GOAL_DEFAULTS` del objetivo vigente, con o sin historial — nunca tuvo progresión propia.
6. Avanza `TrainingProfile.next_focus` al siguiente foco de la rotación del `split_type` del perfil (sin cambios). Desde Hito 9.0, `split_type` mismo se deriva de `TrainingProfile.sessions_per_week` (`TrainingProfile::deriveSplitTypeFromSessionsPerWeek()`, heurística documentada como tal) cada vez que el onboarding lo captura o actualiza — antes de esto, `sessions_per_week` se capturaba y persistía sin ningún efecto real en `TrainingEngine`.
...
**Metadata de foco fino** (Hito 8.4): `Exercise.primary_muscle`/`secondary_muscles` (vocabulario cerrado `App\Training\Enums\MuscleFocus`) es deliberadamente independiente de `muscle_group` (6 valores gruesos, sigue siendo la base del foco de continuidad del punto 3) — permite comparar directamente contra `TrainingProfile.primary_focus`/`secondary_focus` sin traducción. `Exercise.movement_pattern` (`App\Training\Enums\MovementPattern`) se agregó al esquema para que el catálogo real de Hito 9 no requiera otra migración, pero **sin consumidor activo en el scoring todavía**.

## 8.2.1 Capa de contenido multi-proveedor (Hito 9.0/9.1)

**ESTADO ACTUAL**: `TrainingEngine`/`TrainingHandler` solo conocen `Exercise` (el modelo interno) — nunca un proveedor concreto. La cadena completa:

```
ExerciseProviderInterface (Adapter)  →  ExerciseNormalizerInterface  →  Exercise (interno)  →  TrainingEngine
                                                                                    ↓
                                                                          MediaResolver (al enviar)
                                                                                    ↓
                                                                    ExerciseProviderInterface::resolveMedia()
```

- **`App\ExerciseCatalog\Contracts\ExerciseProviderInterface`**: `key()/search()/find()/resolveMedia()/isAvailable()/variants()/alternatives()` — cero concepto exclusivo de un proveedor. `App\ExerciseCatalog\Providers\NullExerciseProvider` (test double, nunca registrado en producción) demuestra desde el día 1 que el contrato no tiene fugas hacia YMove.
- **`provider` es un `string` simple** (nunca un enum de dominio ni una tabla `providers`), validado por `App\ExerciseCatalog\ProviderRegistry` contra `config/exercise_providers.php` — agregar un proveedor nuevo es una clase + una entrada de config, cero cambio en `Exercise`/`TrainingEngine`/migraciones.
- **`App\ExerciseCatalog\Providers\YMove\{YMoveExerciseProvider,YMoveExerciseNormalizer}`**: primer y único proveedor real. El Normalizer traduce el vocabulario de YMove al vocabulario cerrado de WpbotTrainer (`MuscleFocus`, `ExperienceLevel`, `TrackingType`, `Equipment` nuevo) — `difficulty: null` (caso real observado) se propaga tal cual, nunca se inventa.
- **Video nunca se persiste**: `Exercise` guarda únicamente `provider`+`provider_exercise_id`; `App\ExerciseCatalog\MediaResolver::resolve(Exercise)` obtiene la URL fresca en el momento exacto del envío (`TrainingHandler`, reemplazando la lectura antigua de `exercise_snapshot['video_url']`). Un ejercicio manual (`provider=null`, ej. la "Plancha" demo) sigue usando su propia `video_url` estable, sin llamar a ningún proveedor — comportamiento sin cambios respecto a antes de Hito 9.
- **Curación obligatoria de seguridad**: todo ejercicio importado de un proveedor entra con `is_active=false`, `contraindications=null` — `Exercise::activate(User $reviewer)` es la única vía a producción, y rechaza activar si `contraindications` sigue en `null` (mismo convenio `null`/`[]` que `restrictions`/`primary_focus`; `[]` sí es una revisión humana válida).
- **UX de esa curación separada en dos acciones (ver D043)**: en Filament, "Revisar seguridad" (`reviewSafety`) solo guarda `contraindications`, nunca activa, y exige un checkbox de confirmación explícita (regla `accepted`) para poder guardar cualquier valor, incluido `[]`; "Activar" ya no edita `contraindications` y solo es visible cuando `contraindications !== null`. Motivo: `Filament\Forms\Components\TagsInput` (vendor) hidrata cualquier estado no-array como `[]`, así que un formulario que combinara "editar contraindications" con "guardar" podía convertir `null` en `[]` sin ninguna decisión humana real — confirmado empíricamente, no solo teórico. `ExerciseForm.php` retiró el campo editable del guardado estándar por completo (ahora es de solo lectura) por el mismo motivo. `Exercise::activate()` no cambió.
- **`App\ExerciseCatalog\Importer\ExerciseImporter`** + `artisan exercises:sync {provider} {--muscle=*}`: importa metadata únicamente (nunca video); un re-sync nunca toca `is_active`/`contraindications` de un ejercicio ya revisado; un `provider_exercise_id` que deja de aparecer se desactiva (baja suave, nunca borrado).

Ver `docs/DECISIONS.md` D036 para el detalle completo, incluidos los hallazgos reales de esquema (`video_url`/`difficulty_level`/`instructions` nunca fueron nullable) y los riesgos abiertos.

**Resolución de UN ejercicio individual: endpoint directo por id (ver D041, corrige D039/D040)** — `YMoveExerciseProvider::fetchById()` (usada por `find()`/`resolveMedia()`/`variants()`) llama `GET /exercises/{id}` (confirmado en la documentación oficial de YMove — acepta UUID o slug) para resolver UN ejercicio con una única solicitud HTTP, sin recorrer el catálogo paginado en absoluto. `includeVideos=false` para metadata (usado por `find()`, sin costo de cuota); `includeVideos=true` (único caso con costo real) para `resolveMedia()`/`variants()`. La paginación (`searchPaged()`) se mantiene intacta y es exclusiva de `ExerciseImporter::fullSync()` (sincronización del catálogo completo) — ambas rutas son independientes: catálogo → paginación, ejercicio individual → endpoint directo. Encapsulado 100% en el adapter; `ExerciseProviderInterface`/`MediaResolver`/`TrainingEngine`/`TrainingHandler` no cambiaron.

**Observabilidad del resolver de media (post-deploy, ver D040/D041)**: `YMoveExerciseProvider` clasifica cada `null` de `resolveMedia()`/`find()`/`variants()` con `App\ExerciseCatalog\Enums\MediaResolutionReason` (vocabulario agnóstico de proveedor: sin video / cuota excedida / error HTTP / no encontrado / error inesperado) y lo loguea de forma estructurada (`EXERCISE_MEDIA_RESOLUTION`, nunca con secretos ni URLs firmadas) — antes de esto, las 5 causas eran indistinguibles. Incluye el caso real documentado por YMove de "200 con metadata completa pero sin video" cuando se supera el cupo mensual (`_warning.reason = "monthly_exercise_cap"`), clasificado como `provider_quota_exceeded`, nunca confundido con `provider_has_no_video`.

**Equipamiento canónico multi-provider (ver D042)**: `App\Training\Enums\Equipment` (22 valores, completado contra el vocabulario oficial de YMove) es el único vocabulario que comparan `Exercise.equipment_needed` (traducido por cada `ExerciseNormalizerInterface`, ej. `YMoveExerciseNormalizer::mapEquipment()`) y `TrainingProfile.available_equipment` (traducido por `OnboardingConversationService::validateEquipmentArray()` + tabla en el prompt, mismo patrón que `primary_focus`) — `TrainingEngine::isEligible()` nunca cambió, ya comparaba correctamente ambos lados una vez normalizados. Un backfill local (`exercises:backfill-equipment`) re-deriva `equipment_needed` de ejercicios ya sincronizados sin llamar al proveedor.

**"Video validado" — registro propio, nunca `provider_has_video` (ver D042)**: `App\ExerciseCatalog\MediaResolver::resolve()` registra cada resolución EXITOSA de un ejercicio de proveedor en `exercise_video_accesses` (append-only, nunca la URL firmada). `Exercise::videoValidated()` es la única fuente de verdad de "esto ya se probó y funcionó" en Filament — deliberadamente independiente de `provider_has_video` (lo que el proveedor dice tener, nunca confirmado). Un fallo al escribir el registro nunca bloquea la entrega del video.

**Contenido en español, curado por IA en curación — nunca en tiempo de envío (ver D039)**: `Exercise` incorpora `name_es`/`instructions_es`/`important_points_es` (nullable, nunca sobreescriben el original del proveedor) más `content_translated_at`. `App\ExerciseCatalog\Curation\ExerciseSpanishContentGenerator` (100% agnóstico de proveedor, opera solo sobre los campos ya normalizados de `Exercise`) traduce/adapta reutilizando `AiServiceInterface::getResponse()` y valida el resultado por código antes de aceptarlo — rechaza (`SpanishContentGenerationException`) cualquier traducción cuyo número de pasos/puntos importantes no coincida exactamente con el original, nunca confía la fidelidad estructural solo al prompt. `Exercise::toSnapshot()` prefiere `*_es` sobre el original cuando existe — es la ÚNICA pieza de este fix con efecto en el envío real (la IA nunca se invoca al enviar un mensaje; la traducción ya existe de antemano). Como la curación de `Exercise` es una acción global sin `Tenant` asociado y `AIServiceFactory::make()` solo existe con alcance por Tenant, la acción de Filament pide explícitamente de qué `Tenant` tomar la credencial — ver D039, punto 3, para el gap arquitectónico completo.

**Sincronización completa del catálogo (Hito 9.3, ver D038)**: `ExerciseImporter::fullSync(providerKey, hasVideo=null, muscleFocus=null)` es la vía para un inventario de referencia real del proveedor completo (no solo un lote curado a mano) — recorre TODAS las páginas usando la paginación que el proveedor reporta (`ExerciseProviderInterface::searchPaged()`), nunca un número asumido. Reconcilia bajas (desactiva, nunca borra) **solo tras un recorrido 100% completo y sin errores**; un fallo a mitad de camino nunca se trata como "el ejercicio desapareció" (`ProviderSyncException`). `exercises:sync --muscle=X` corre sobre este mismo método, con la reconciliación acotada al músculo solicitado. `provider_has_video` (columna nueva, nullable) persiste la señal cruda del proveedor sobre disponibilidad de video — refrescada en cada re-sync, sin relación con `is_active`. Catálogo real sincronizado: 1068 ejercicios, 1067 con video, 1 sin video, todos `is_active=false` pendientes de curación.

**Técnica de ejecución por ejercicio (Hito 9.2, ver D037)**: `Exercise` incorpora `instructions` (json, array de pasos — cambió de `text`), `important_points`, `common_mistakes` y `breathing_cue` (los 3 últimos nullable). `instructions`/`important_points` vienen del proveedor y se refrescan en cada re-sync; `common_mistakes`/`breathing_cue` NUNCA los provee YMove — solo existen si un administrador los cura, y `ExerciseImporter` los preserva en cada re-sync exactamente igual que `contraindications`. Asimetría de curación deliberada: `contraindications=null` **e `instructions` vacío** bloquean `Exercise::activate()`; los otros dos nunca bloquean. `App\Training\Support\ExerciseMessageFormatter` arma el texto que el usuario recibe por ejercicio (técnica + respiración + precauciones), leyendo exclusivamente de `WorkoutExercise.exercise_snapshot` — nunca del `Exercise` en vivo. `TrainingHandler` envía una cabecera mínima de sesión + un mensaje de técnica y un video por ejercicio (reemplaza el resumen combinado detallado que existía antes).
...
TrainingHandler::handle(ExecutionContext)
  ...
  → ¿TrainingProfile::isOnboardingComplete(Contact)? (Hito 8.3: firma recibe Contact — el nombre vive ahí)
       NO → ContextBuilder->build(['training_profile']) + OnboardingConversationService::extractAndRespond()
            ...
            → aplica los campos válidos y no nulos (incluido Contact.customer_name si vino y aún no existía)
            → si el campo pendiente ANTES de este turno era 'physical_stats', se marca
              TrainingProfile.physical_stats_asked=true sin importar qué llegó (Hito 8.3 — "preguntar
              una sola vez", nunca bloqueante)
...
- **Extract + Narrate en una llamada** (`extractAndRespond()`): el LLM devuelve `{extracted: {...}, next_action, response}`. Desde el Hito 8.4, `extracted` incluye 15 campos: `name` (persiste en `Contact.customer_name`, nunca en `TrainingProfile`), `goal`, `experience_level`, `primary_focus`, `secondary_focus`, `training_location`, `available_equipment`, `equipment_fully_equipped`, `restrictions`, `sessions_per_week`, `age`, `sex`, `weight_kg`, `height_cm`, `safety_signal_text` — todos validados contra su vocabulario/forma permitida antes de aceptarse (enums, arrays de strings, rangos numéricos). `next_action` (uno de 9 valores `ask_*` o `complete_onboarding`) y `response` (texto natural candidato) son señales adicionales, nunca autoritativas.
- **`primary_focus`/`secondary_focus`** (Hito 8.4, ver D034): traducción de lenguaje natural ("quiero aumentar glúteos", "piernas", "espalda") al vocabulario cerrado `MuscleFocus` vía una tabla explícita en el prompt — `validateMuscleFocusArray()` descarta cualquier valor fuera de ese vocabulario. `primary_focus` tiene pregunta dedicada obligatoria (no puramente oportunista); `secondary_focus` es representación interna — el prompt instruye explícitamente nunca mencionar estos términos técnicos al usuario. Mismo criterio `null`/`[]` que `restrictions`/`available_equipment`.
- **`equipment_fully_equipped`** (Hito 8.3, hallazgo real del E2E comercial): representa una declaración de disponibilidad amplia/completa sin enumerar ("tengo de todo", "lo normal de un gimnasio") — el prompt distingue explícitamente esto de mencionar equipo específico ("solo pesas") o solo el lugar sin hablar de equipo. Nunca asume que un `training_location=gym` implica automáticamente todo el equipo posible — solo registra lo que el usuario declaró.
- **Datos físicos nunca bloqueantes** (Hito 8.3, decisión de producto explícita): `age`/`sex`/`weight_kg`/`height_cm` se capturan si el usuario los da, pero `TrainingProfile.physical_stats_asked` (no los valores en sí) es lo único que el gate de completitud verifica — se pregunta una sola vez, se acepta cualquier respuesta (parcial o declinada), nunca se vuelve a insistir. Sin ningún consumidor en `TrainingEngine` todavía — ver sección 9.
...
- **Señal de "pregunta pendiente" en la extracción (fix post-E2E, ver D039)**: `extractAndRespond()` recibe un cuarto parámetro opcional `$pendingField` — el mismo campo que `TrainingProfile::firstMissingOnboardingField()` ya calculaba antes del turno, reenviado por `TrainingHandler` como una sola línea de contexto en el prompt. Nunca cambia quién decide qué falta de verdad (sigue siendo 100% código); solo le da a la IA el contexto de a qué pregunta responde un mensaje corto y ambiguo. Con esa señal, dos reglas generalizadas en el prompt: una respuesta de tipo foco a la pregunta de objetivo se extrae en `primary_focus`/`secondary_focus` (reutilizando la tabla de traducción ya existente) dejando `goal` en `null`, nunca forzado; y cualquier negación natural a la pregunta de restricciones se acepta como `restrictions: []`, nunca queda sin extraer.
...
- **Fix real (Hito 8.3, hallazgo del E2E comercial)**: una confirmación breve sin detalle ("hecho", "listo", "ya") **debe** producir un reporte (con `exercise_name: null`, `sets: []`), nunca `reports: []` — el prompt lo instruye explícitamente. Antes de este fix, el prompt no cubría este caso; lo más probable es que el modelo devolviera "sin reportes", lo que `TrainingHandler` interpretaba como "sin señal" y dejaba pasar al flujo normal — que, al ser idempotente (`TrainingEngine::decideNextSession()` devuelve la sesión `scheduled` existente sin cambios), reenviaba el mismo ejercicio/video. La resolución del ejercicio único pendiente sin nombre explícito (siguiente punto) **ya existía y era correcta** — nunca se alcanzaba.
- `skip_reason` (Hito 8.3, enum `App\Training\Enums\SkipReason`): solo se completa cuando `not_performed=true` y el usuario dio o insinuó una razón ("no pude" → `cant_do`, "no quiero" → `dont_want`, "no me dio tiempo" → `no_time`) — nunca inventado. Dato recopilado desde MVP para aprendizaje futuro (Hito 8.2, Parte 13), sin ningún consumidor todavía.
