...
4. **Selecciona hasta 3 ejercicios** (`selectExercises()`, reescrito en Hito 8.4 — ver D034) en el orden de prioridad **elegibilidad → foco declarado → objetivo/nivel → anti-repetición → determinismo**:
   - **Elegibilidad** (filtro duro, `isEligible()`): excluye cualquier ejercicio cuyas `contraindications` choquen con `restrictions`, o cuyo `equipment_needed` no esté cubierto por `available_equipment` — salvo que `TrainingProfile.equipment_fully_equipped=true`, que hace elegible cualquier ejercicio en cuanto a equipo.
...
6. Avanza `TrainingProfile.next_focus` al siguiente foco de la rotación del `split_type` del perfil (sin cambios).
