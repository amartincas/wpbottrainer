...
| **UX de curación de seguridad: `reviewSafety`/`activate` separados (D043)** | `ExerciseResourceTest.php` (+9, +2 actualizados) — sin el checkbox de confirmación explícita, "Revisar seguridad" falla su validación y no guarda nada (ni con `contraindications=[]`); con confirmación, guarda `[]` o una lista real, y nunca activa el ejercicio; oculto una vez ya activo. "Activar" oculto mientras `contraindications===null` (probado en la tabla y en el header de edición), visible una vez revisado, sigue delegando 100% en `Exercise::activate()`. **Prueba empírica** de que guardar el formulario de edición estándar sin tocar `contraindications` ya no lo cambia (antes del fix, `Filament\Forms\Components\TagsInput` lo convertía de `null` a `[]` al hidratarse — confirmado leyendo el vendor y con este mismo test). Filtro ad-hoc `specific_ids` aísla un lote de ids sin mezclarlo con el resto del catálogo `pending_review`; y (fix posterior, ver adenda D043) un texto no vacío sin ningún id numérico válido (ej. `"1-53"`) devuelve cero resultados, nunca el catálogo sin acotar. |
| **Fixes reales encontrados durante la curación manual de los 53 (D044)** | `ExerciseResourceTest.php` (+1) — generar contenido en español desde el encabezado de la página de edición refresca el formulario ya abierto (`$this->fillForm()`), así que un "Save" posterior conserva la traducción en vez de sobrescribirla con el estado vacío de antes de generar; este test falló primero contra un intento de fix con `refreshFormData()` (evidencia empírica real de que ese método no funciona para campos array como `instructions_es`, por cómo `Schema::fillPartially()` aplana el estado con `dot()`), quedando como regresión real, no teórica. `tests/Feature/Services/AI/GrokServiceTest.php` (nuevo, +3) — `GrokService::getResponse()` funciona normal sin fallos; reintenta y tiene éxito tras un timeout simulado (`ConnectionException`, reproduce el error real "cURL error 28"); da un error claro tras agotar los reintentos. `analyzeImage()` (Payments, visión) queda cubierto por su suite existente, sin cambios. **Tercer hallazgo (adenda D044)**: `ExerciseResourceTest.php` (+1) — "Generar contenido en español" sigue visible (tabla y edición) para un ejercicio inactivo con `contraindications` ya revisado (`reviewStatus()==='inactive'`), no solo en `'pending_review'` — antes de este fix quedaba bloqueada para siempre en cuanto se guardaba cualquier revisión de seguridad. |
