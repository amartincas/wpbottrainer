...
- **`contraindications` NUNCA es un campo editable del formulario estándar de `ExerciseResource` (D043).** Solo se edita a través de la acción "Revisar seguridad" (`reviewSafety`, en `ExercisesTable`/`EditExercise`), que exige un checkbox de confirmación explícita (`->rule('accepted')`) antes de guardar cualquier valor. Si necesitas exponer otro campo "sensible" similar (bloquea `activate()` o cambia el estado de revisión), no lo pongas en el schema del formulario estándar — `Filament\Forms\Components\TagsInput` (y componentes de array similares) hidratan `null` como `[]` automáticamente en el navegador, así que un formulario de guardado genérico puede persistir una "revisión" que nadie hizo conscientemente. Usa una acción dedicada con confirmación explícita, como aquí.
- **Una acción de encabezado de `EditRecord` que actualiza `$record` directamente (`$record->update(...)`) DEBE refrescar el formulario ya montado en pantalla, o un "Save" posterior lo sobrescribe con el estado viejo (D044, hallazgo real con `generateSpanishContent`).** Usa `$this->fillForm()` (protegido, heredado de `EditRecord`) para un refresco completo — es lo mismo que se ejecuta al montar la página, sin sorpresas. **No uses `refreshFormData($statePaths)` con la ruta plana de un campo array** (ej. `'instructions_es'`) esperando que se actualice solo — `Schema::fillPartially()` aplana el estado con `collect($state)->dot()` antes de filtrar, así que un campo array termina como `instructions_es.0`, `instructions_es.1`... y la ruta plana ya no coincide con nada; solo funciona así para campos escalares (`name_es` sí, `instructions_es` no) — confirmado con un test, no una suposición.
- **Cualquier llamada a `AiServiceInterface::getResponse()`/`GrokService` que dependa de una respuesta larga (traducción de varios pasos, no un chat corto) necesita su propio `->timeout()` explícito — el default de Laravel son 30s (D044, hallazgo real: "cURL error 28... 30002 milliseconds").** `GrokService::getResponse()` ya usa `->timeout(90)->retry(2, 2000)`; si agregas un consumidor nuevo con una carga de trabajo similarmente larga, no asumas que 30s por defecto alcanza.
- **No condiciones la visibilidad de una acción de Filament a `Exercise::reviewStatus()` cuando en realidad solo te interesa `is_active` (D044, adenda).** `reviewStatus()` combina `is_active` Y `contraindications` en un solo valor (`pending_review`/`active`/`inactive`) — útil para mostrar un estado de flujo, pero peligroso como condición de `visible()`: exigir `reviewStatus()==='pending_review'` bloquea una acción para siempre en cuanto `contraindications` deja de ser `null`, aunque el ejercicio nunca se active (hallazgo real: "Generar contenido en español" quedó inaccesible para el ID 55 tras guardar su revisión de seguridad). Si la acción solo debe estar disponible mientras el ejercicio no esté activo, usa `! $record->is_active` directamente — no un valor derivado que mezcla otro concepto.
...
- **Cualquier fallo de resolución de media debe clasificarse con `App\ExerciseCatalog\Enums\MediaResolutionReason`, nunca con un `null` sin explicación (D040/D041).** El vocabulario es agnóstico de proveedor a propósito — si agregas un Adapter nuevo, reutiliza este enum para tu propio logging en vez de inventar tus propios códigos de motivo.
- **Resolver UN ejercicio individual (`find()`/`resolveMedia()`/`variants()`) usa el endpoint directo por id del proveedor cuando existe, nunca recorre el catálogo paginado (D041).** `YMoveExerciseProvider::fetchById()` llama `GET /exercises/{id}` — confirmado en la documentación oficial de YMove. `searchPaged()`/la paginación siguen existiendo, pero exclusivamente para `ExerciseImporter::fullSync()` (sincronización del catálogo completo), nunca para resolver un id puntual. Si agregas un proveedor nuevo, verifica primero en su documentación si existe un endpoint equivalente por id antes de asumir que hace falta "listar y buscar" — ese fue exactamente el error de diseño corregido en D041.
- **`App\Training\Enums\Equipment` es el ÚNICO vocabulario de equipamiento — nunca compares texto libre contra `equipment_needed` (D042).** Si un proveedor nuevo necesita mapear su propio equipamiento crudo, hazlo en su Normalizer (mismo patrón que `YMoveExerciseNormalizer::mapEquipment()`); si el onboarding necesita reconocer una frase nueva del usuario, agrégala a la tabla de traducción de `OnboardingConversationService`, nunca compares el string sin traducir.
- **`provider_has_video` (lo que el proveedor DICE) y `Exercise::videoValidated()` (lo que NOSOTROS comprobamos, vía `exercise_video_accesses`) son preguntas distintas — nunca uses una por la otra (D042).** Cualquier indicador de "video listo/confirmado" en Filament debe usar `videoValidated()`.
